...
class Bookingssystem extends AppModel
{
    ...
    public function updateBooking($data) {
        ...
        if(!empty($data['BookingTrack'])) {
            //            $this->BookingTrack->deleteAll(array('BookingTrack.booking_id' => $data['Booking']['id']));
    ...
    function checkAndCloseTrack(){
        ...
        $this->EmailQueue->testMail(array(
            'email'     => 'soyab@blackidsolutions.com',
            'subject'   => date('d-m-Y h:i:s A') .' :: checkAndCloseTrack www.in-demo.dk',
    ...
    public function bookingBeforeThreeHourNotification() {
        $this->EmailQueue           = ClassRegistry::init('EmailQueue');
        $this->SmsQueue             = ClassRegistry::init('SmsQueue');
        $this->User                 = ClassRegistry::init('User');
        $this->Track                = ClassRegistry::init('Track');
        ...
        $this->EmailQueue->testMail(array(
            'email'     => 'ranasoyab@gmail.com',
            'subject'   => date('d-m-Y h:i:s A') .' :: CRON WORKING elev-system Before 3 Hours ',
            'data'      => 'Test Mail',
        ));

        echo '<pre>';
        print_r( $studentStartTime );
        echo '</pre>';
        
        echo '<pre>';
        print_r( $studentEndTime );
        echo '</pre>';
