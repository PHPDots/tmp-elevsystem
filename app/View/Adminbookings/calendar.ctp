...
<script type="text/javascript">
        ...
        jQuery.each(tracks,function(index,value) {
            ...
            html += '<div class="span12 no-margin trackDetails" id="trackDetails_'+index+'">';
                html += '<div class="span2 no-margin trackCheckboxCt">';
                    ...
                    html += '<input type="hidden" name="data[BookingTrack]['+index+'][track_id]"  value="'+index+'" class="cust_check"/>';
                    html += '<label for="chk_'+index+'">'+value+'</label>';
                html += '</div>';
            
                html += '<div class="bookingTrack">';
                    html += '<div class="timeSlot span2" style="display:none;">';
                        html += '<select name="data[BookingTrack][%i%][time_slot][]" class="timeSlotSelectCt span12" id="timeSlot_'+index+'">';
                        html += '</select>';
                    html += '</div>';
                    ...
                    html += '<div class="studentDiv span8" id="studentDiv'+index+'" style="display:none;">';
                        ...
                        html += '</div>';
                        html += '<div class="studentId_'+index+'_std_error std_error"></div>';
                        /*
                        html += '<div class="span2 no-margin trackCheckboxCt">';
    ...
    function dayEvent(area,date,week) {
        ...
        jQuery.ajax({
            url         : '<?php echo $this->Html->url(array('controller'   => 'adminbookings','action'  => 'getBookings')); ?>',
    ...
    jQuery(document).ready(function(){
        ...
        jQuery('.areaObject').click(function() {
            if(jQuery(this).is(':checked')) {
                ...
                <?php } ?>                
                jQuery.ajax({
                    url         : '<?php echo $this->Html->url(array('controller'=>'adminbookings','action'=>'calendar')); ?>',
        ...
        jQuery(document).delegate('.bookingLink','click',function() {
            ...
            jQuery('#BookingCalendarForm').attr('action','<?php echo $this->Html->url(array('controller' => 'adminbookings','action' => 'add','?' => array('iframe' => (isset($this->request->query['iframe']) ? $this->request->query['iframe'] : 0)))); ?>');
            element.html(jQuery('#bookingAddForm').html()).show();
            jQuery.scrollTo('#bookingForm','slow');
            jQuery.ajax({
                url         : '<?php echo $this->Html->url(array('controller' => 'adminbookings','action' => 'getTimeBookings')) ?>',
                type        : 'get',
                dataType    : 'json',
                ...
                success     : function(data) {
                    ...
                    jQuery.each(data,function(id,value) {
                        jQuery.each(value,function(index,booking) {
                            ...
                            jQuery('.tracks').find('#trackDetails_'+index).find('#chk_'+index).removeAttr('disabled');
                        });
                    });
        ...
        jQuery(document).delegate('.trackCheckbox','click',function() {
            var current_chk_name = jQuery(this).attr('name');
            var user_id_name = current_chk_name.replace('track_id','booking_user_id');
            jQuery(this).siblings('.remove_user_id').remove();
            jQuery(this).parent(".checked").append('<input type="hidden" class="remove_user_id" name="'+user_id_name+'" value="'+<?php echo $currentUser['User']['id']; ?>+'"> ');
        ...
        jQuery(document).delegate('.bookingLinkName','click',function() {
            ...
            /*setTimeout(function() {
            jQuery.ajax({
                url         : '<?php echo $this->Html->url(array('controller' => 'adminbookings','action' => 'getTimeBookings')) ?>/'+bookingId,
                type        : 'get',
                dataType    : 'json',
                data        : 'area='+area+'&date='+date+'&time='+timeSlot,
                success     : function(data) {
        ...
        jQuery(document).delegate('.studentIdAutoSuggest','keyup',function() {
            ...
            if(jQuery(this).val() == '') {
                ...
                jQuery('.'+studentField+'_std_error').html('');
            ...
            jQuery(this).autocomplete({
                ...
                select: function( event, ui ) {
                    ...
                    if(ui.item.balance > 0){
                        jQuery('.'+studentField+'_std_error').html(ui.item.label+' Elevens saldo er skyldig med '+ ui.item.balance +' kr')
                    }
        ...
        jQuery(document).delegate('.releaseTrack','click',function(){
            ...
            var ele         = jQuery(this);
            jQuery.fancybox({
                'href'          : '<?php echo $this->Html->url(array('controller'   => 'adminbookings' , 'action' => 'releaseTrack')); ?>?area='+area+'&date='+date+'&time_slot='+time,
                'width'         : '75%',
                'height'        : '75%',
                'autoScale'     : false,
                'transitionIn'  : 'none',
                ...
                'beforeClose'   : function () {
                    if(sessionStorage.getItem('closeFancybox') == 'closed') {
                        ...
                        if(ele.hasClass('reopenTracks')) {
                            jQuery.ajax({
                                url         : '<?php echo $this->Html->url(array('controller'   => 'adminbookings','action'  => 'reopenTracks')); ?>',
                            ...
                            // window.location = "<?php echo $this->Html->url(array('controller' => 'adminusers','action' => 'logout')); ?>";
                            location.reload();
                        }
                    } else {
                        window.location='<?php echo $this->Html->url(array('controller' => 'adminbookings','action'=>'calendar')); ?>?date='+date+'&area='+area;
                    }
                }
            });
        });
        ...
        jQuery(document).delegate('.closeTrack','click',function(){
            ...
            var ele     = jQuery(this);
            jQuery.ajax({
                url         : '<?php echo $this->Html->url(array('controller'   => 'adminbookings','action'  => 'closeTracks')); ?>',
        ...
        jQuery(document).delegate('.editTrack','click',function(){
            ...
            var time    = jQuery(this).attr('time_slot');
            var ele     = jQuery(this);
            var url     = '<?php echo $this->Html->url(array('controller' => 'adminbookings','action'  => 'editTracks')); ?>?date='+date+'&area='+area+'&time='+time;
            jQuery.ajax({
                url         : '<?php echo $this->Html->url(array('controller'   => 'adminbookings','action'  => 'editTracks')); ?>',
                data        : 'date='+date+'&area='+area+'&time='+time,
                dataType    : 'html',
                complete    : function() {
        ...
        jQuery(document).delegate('.editTrackDetails','click',function(){
            ...
            var time    = jQuery(this).attr('time_slot');
            var ele     = jQuery(this);
            var url     = '<?php echo $this->Html->url(array('controller' => 'adminbookings','action'  => 'editBookings')); ?>?date='+date+'&area='+area+'&time='+time;
            jQuery.ajax({
                url         : '<?php echo $this->Html->url(array('controller'   => 'adminbookings','action'  => 'editBookings')); ?>',
                data        : 'date='+date+'&area='+area+'&time='+time,
                dataType    : 'html',
                complete    : function() {
        ...
        jQuery(document).delegate('.delete_track','click',function() {
            var ele = jQuery(this);
            jQuery.ajax({
                url         : '<?php echo $this->Html->url(array('controller'   => 'adminbookings','action'  => 'deleteTrack')); ?>/'+jQuery(this).attr('booking-id')+'/'+jQuery(this).attr('track-id'),
                dataType    : 'json',
                beforeSend  : function() {
...
function track_lock(){
        ...
        buttons         : {
            'Ok'        : function(){
                ...
                window.location.href = "<?php echo $this->Html->url(array('controller' => 'adminbookings','action'=>'calendar')); ?>";
                }
        }
    }); 
        return false;
