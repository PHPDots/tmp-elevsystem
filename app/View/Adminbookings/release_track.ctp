...
jQuery(document).ready(function() {
    ...
    jQuery('#releaseTrackConf').dialog({
            autoOpen        : false,
            modal           : true,
            width           : 400,
            close           : function(ev, ui) { 
                jQuery('#releaseTrackConf').dialog('close');
            }
        });
    ...
    jQuery(document).delegate('#updateTrackBtn','click',function(){
        ...
        href = href.replace("bookings/","adminbookings/");

        var elt = document.getElementById('BookingCourse');

        var area        = elt.options[elt.selectedIndex].text;
            var date        = "<?php echo date('d.m.Y',strtotime($this->request->query['date'])); ?>";
            jQuery(".set_r_date").html(date);
            jQuery(".set_r_c_name").html(area);
        ...
        jQuery('#releaseTrackConf').dialog('option','buttons',buttons);
        jQuery('#releaseTrackConf').dialog('open');
...
<?php $this->end(); ?>
<div id="releaseTrackConf" style="display: none;" title="Frigive banen">
    Er du sikker på at du vil frigive banen <span class="set_r_date">Date</span> med det følgende kursus <span class="set_r_c_name" style="font-weight: bold;">Name of course</span>
</div>
    ...
    <div class="row-fluid"><div class="widget">
        ...
        <div class="widget-content no-padding">
            ...
            <div class="form-row">
                ...
                $teacher_id = ($bookings[$key]['booking_user_id'] != '') ? $bookings[$key]['booking_user_id'] : $bookings[$key]['user_id'];
                echo (isset($drivingSchool[$users[$teacher_id]['company_id']]))? $drivingSchool[$users[$teacher_id]['company_id']] : ''; ?></label>
