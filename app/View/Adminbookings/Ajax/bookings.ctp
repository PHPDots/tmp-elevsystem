        ...
        foreach($finalBookingDetails as $finalBookingDetail) {
            ...
            <tr class="<?php echo (++$i%2==0)?'even':'odd';?>">
                <?php 
                    ...
                    foreach ($finalBookingDetail as $bookingDetail) {   
                        ...
                        if(isset($bookingDetail['key']) && !empty($bookingDetail['key'])) {
                            ...
                            $link       = $this->Html->url(array('controller' => 'adminbookings','action'  => 'edit',$relatedBookingId));
                            ...
                            if($relatedBookingId == ''){
                                ...
                                $url    = '<a href="javascript:" url="'.$link.'" class="bookingLinkName '.$deactiveClass.'" area="'.$area.'" date="'.$date.'" time="'.$finalBookingDetail[0]['key'].'" ">';
                                $url   .= '<i class="fa fa-plus-circle"></i>';
                                $url   .= "</a>";
                            }
                            ...
                            $link       = $this->Html->url(array('controller' => 'adminbookings','action'  => 'edit',$bookingDetail['id']));
                            ...
                            $endtimestamp = strtotime($CurdateTime);
                            $difference = round(($endtimestamp - $starttimestamp)/3600);
                            $is_abel_to_edit = false;
                            if(($difference <= 4 && $difference > 0) || $difference < -72  ){
                                $is_abel_to_edit = true;
                            }
                            ...
                            if(($userDetails[$bookingDetail['teacher']]['role'] == 'external_teacher') && ($bookingDetail['user'] == -1)) {
                                ...
                                if($bookingDetail['booking_user_id'] != ''){
                                    $username   = $userDetails[$bookingDetail['booking_user_id']]['firstname'].' '.$userDetails[$bookingDetail['booking_user_id']]['lastname'];
                                    if(!empty($userDetails[$bookingDetail['booking_user_id']]['company'])) {
                                        $username  .= ' - '.$userDetails[$bookingDetail['booking_user_id']]['company'];
                                    }
                                }
                                ...
                                if($bookingDetail['booking_user_id'] != ''){
                                    $firstname  = (empty($userDetails[$bookingDetail['booking_user_id']]['nick_name_user']))?$userDetails[$bookingDetail['booking_user_id']]['firstname']:$userDetails[$bookingDetail['booking_user_id']]['nick_name_user'];
                                    if(!empty($userDetails[$bookingDetail['booking_user_id']]['company_id'])) {
                                        $firstname .= ' ( '.$userDetails[$bookingDetail['booking_user_id']]['company_id'].' )';
                                    }
                                }
                                ...
                                $url   .=  ' ]';
                                // $url   .= $dateTime.'---'.$CurdateTime.'---->>>'.$difference;
                                $url   .= "</a>";
                            } else {
                                $username   = $userDetails[$bookingDetail['teacher']]['firstname'].' '.$userDetails[$bookingDetail['teacher']]['lastname'];
                ...
                <td align="center" width="20%">
                    <?php
                        $ipAddresses = array($released_ip_address,'171.50.203.8','180.211.118.242','192.168.1.29','::1','106.201.237.251');
