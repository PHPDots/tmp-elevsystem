                ...
                <div class="form-row no-padding" id="studentsCt">
                    <div class="widget-content no-padding">
                        ...
                        <div class="form-row <?php echo (isset($users[$bookingTrack['student_id']]) && !empty($users[$bookingTrack['student_id']]['date_of_birth'])) ? 'addBackground' : ((!empty($bookingTrack['date_of_birth'])) ? 'addBackground' : '');?>">
                            ...
                            <label class="span2">
                                <?php 
                                echo (!isset($users[$teacher[$bookingTrack['booking_id']]['user_id']])) ? '' : 
                                    $users[$teacher[$bookingTrack['booking_id']]['user_id']]['firstname'].' '.$users[$teacher[$bookingTrack['booking_id']]['user_id']]['lastname']; 
                                ?>
                            </label>
                            <label class="span1">
                                <?php
                                echo (!isset($users[$teacher[$bookingTrack['booking_id']]['user_id']]) || empty($users[$teacher[$bookingTrack['booking_id']]['user_id']]['company_id'])) ? '' : $drivingSchools[$users[$teacher[$bookingTrack['booking_id']]['user_id']]['company_id']];
                                ?>
                            </label>
                            <?php
                            $status = '';
