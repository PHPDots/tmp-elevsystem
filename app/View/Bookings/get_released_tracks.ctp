    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function(){
            ...
            jQuery('.viewStudent').click(function() {
                ...
                jQuery.ajax({
                    url         : '<?php echo $this->Html->url(array('controller' => 'bookings','action' => 'updateGetReleasedTrackStatus')) ?>?id=' + bookingTrackId,
                    type        : 'get',
                    dataType    : 'JSON',
                    success     : function(data) {
                        if(data.status == 1){
            ...
            jQuery('.student_name').autocomplete({
                ...
                select: function( event, ui ) {
                    ...
                    jQuery.ajax({
                        url         :  '<?php echo $this->Html->url(array('controller' => 'users','action' => 'getStudentInfo')) ?>',
            ...
            jQuery(document).delegate('.updateUser','click',function() {
                jQuery.ajax({
                    url         : '<?php echo $this->Html->url(array('controller' => 'bookings','action' => 'updateTrackUser')) ?>/'+false,
            ...
            jQuery(document).delegate('.addStudentBtn','click',function(){
                jQuery.fancybox({
                    'href'          : '<?php echo $this->Html->url(array('controller'   => 'users' , 'action' => 'add')); ?>/student/layout',
                    'width'         : '90%',
                    'height'        : '90%',
                    'autoScale'     : false,
                    'transitionIn'  : 'none',
                ...
                <div class="form-row no-padding" id="studentsCt">
                    <div class="widget-content no-padding">
                        ...
                        <div class="form-row <?php echo (isset($users[$bookingTrack['student_id']]) && !empty($users[$bookingTrack['student_id']]['date_of_birth'])) ? 'addBackground' : ((!empty($bookingTrack['date_of_birth'])) ? 'addBackground' : '');?>">
                            ...
                            <label class="span2"><?php echo (!isset($users[$teacher[$bookingTrack['booking_id']]['user_id']]))?'':$users[$teacher[$bookingTrack['booking_id']]['user_id']]['firstname'].' '.$users[$teacher[$bookingTrack['booking_id']]['user_id']]['lastname']; ?></label>
                            <label class="span2"><?php echo (!isset($users[$teacher[$bookingTrack['booking_id']]['user_id']]) || empty($users[$teacher[$bookingTrack['booking_id']]['user_id']]['company_id']))?'':$drivingSchools[$users[$teacher[$bookingTrack['booking_id']]['user_id']]['company_id']]; ?></label>
